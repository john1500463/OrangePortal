...
public partial class Expedite_multiple_incidents : System.Web.UI.Page
{
    ...
    protected void showgrid(ArrayList arr)
    {
        ...
        try
        {
            ...
            SqlCommand command = new SqlCommand();
            command.Connection = conn;
            String cmdstring = "Select [INC Incident Number],[INC Status],[INC DS Submit Date],[AG Assigned Group Name],[AG Assignee],[INC DS Last Modified Date] From [dbo].['All_Incidents'] where [INC Incident Number]='" + arr[0] + "'";
